...
impl GameObject {
    pub fn new(body: RigidBody, object_type: GameObjectType) -> Self {
        let color = match object_type {
            GameObjectType::Wall => [0.5, 0.5, 0.5, 1.0],
            GameObjectType::Paddle => {
                if body.id == "paddle1" {
                    [0.0, 1.0, 0.0, 0.5] // Semi-transparent for paddle1
                } else {
                    [0.0, 1.0, 0.0, 1.0]
                }
            }
            GameObjectType::Ball => [1.0, 1.0, 1.0, 1.0],
        };
...
impl GameState {
    ...
    pub fn reset_ball(game_objects: &mut [GameObject]) {
        let ball_index = game_objects
            .iter()
            .position(|obj| obj.object_type == GameObjectType::Ball)
            .unwrap();
